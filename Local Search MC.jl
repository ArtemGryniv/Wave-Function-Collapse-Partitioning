...
function report_LS(graph, edges, nodes)
    length(edges) < 63 && viewgraph(graph, nodes) 
end


################################################
function local_search_MC(graph)
    ...
    while true
        ...
        for node in nodes
            ...
            if set1_count > set2_count && node.set != 2
                delete!(sets[node.set], node)
                push!(sets[2], node)
                node.set = 2
                moved = true
            elseif set2_count > set1_count && node.set != 1
                delete!(sets[node.set], node)
                push!(sets[1], node)
                node.set = 1
                moved = true
...
function main(graph, edges, amt_trials = 1000)   
    ...
    for _ in 1:amt_trials
        sets, nodes = local_search_MC(graph)
...
function benchmark(graph, edges)
    subsets, edges = local_search_MC(graph)
end
